 ...
 * merge-into-current-settings rule and the value guards are the core's. Every refusal is typed
 * (GH-617): the core's `INVALID_ARGUMENT` for a value out of range, and `INVALID_ARGUMENT` naming
 * the verb for a flag combination the grammar refuses, so the CLI envelope never says `OTHER`.
 */
object AppearanceOps:

  private def refuse(verb: String, reason: String): XLResult[Sheet] =
    Left(XLError.InvalidArgument(verb, reason))
   ...
   */
  def applyAutoFilter(sheet: Sheet, range: Option[CellRange], clear: Boolean): XLResult[Sheet] =
    (range, clear) match
      case (Some(_), true) => refuse("autofilter", "<range> and --clear are mutually exclusive")
  ...
  def applySheetView(
    ...
    if gridlines.isEmpty && zoom.isEmpty && tabSelected.isEmpty then
      refuse("sheet-view", "at least one of gridlines, zoom, tab-selected is required")
    else sheet.mergeSheetView(gridlines, zoom, tabSelected)
   ...
   */
  def applyTabColor(sheet: Sheet, colorStr: Option[String], clear: Boolean): XLResult[Sheet] =
    (colorStr, clear) match
      case (Some(_), true) => refuse("tab-color", "<color> and --clear are mutually exclusive")
      case (None, false) => refuse("tab-color", "a <color> argument or --clear is required")
      case (Some(s), false) =>
        ColorParser.parse(s).map(sheet.withTabColor).left.map(XLError.ColorError(s, _))
  ...
  def applyPageSetup(
    ...
    then
      refuse(
        "page-setup",
        "at least one of orientation, scale, fit-to-width, fit-to-height, fit-to-page is required"
      )
    else sheet.mergePageSetup(orientation, scale, fitToWidth, fitToHeight, fitToPage)
  ...
  def applyHeaderFooter(
    ...
    if !anyText && !differentOddEven && !differentFirst then
      refuse(
        "header-footer",
        "at least one of odd-header, odd-footer, even-header, even-footer, first-header, " +
          "first-footer, different-odd-even, different-first is required"
      )
    else
      Right(
