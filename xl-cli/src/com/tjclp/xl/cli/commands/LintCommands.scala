...
object LintCommands:
  ...
  /**
   * GH-663: the formula oracle behind `formula-unparseable` — the evaluator's parser, shared with
   * the aggregate module's `Excel.lint` (GH-674) so a script lints what `xl lint` lints. What is
   * and is not a finding is documented on [[UnparseableFormula]].
   */
  val formulaCheck: WorkbookLint.FormulaCheck = UnparseableFormula.check
