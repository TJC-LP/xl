  ...
  def parseThemeElement(elem: Elem): Either[String, ThemePalette] =
    ...
    clrScheme match
      ...
      case Some(cs) =>
        ...
        // Accent fallbacks share DefaultTheme.accents — the written theme part and the
        // defaulted palette can never disagree (GH-407)
        val accent1 = extractColor(cs, "accent1").getOrElse(DefaultTheme.accentArgb(0))
        val accent2 = extractColor(cs, "accent2").getOrElse(DefaultTheme.accentArgb(1))
        val accent3 = extractColor(cs, "accent3").getOrElse(DefaultTheme.accentArgb(2))
        val accent4 = extractColor(cs, "accent4").getOrElse(DefaultTheme.accentArgb(3))
        val accent5 = extractColor(cs, "accent5").getOrElse(DefaultTheme.accentArgb(4))
        val accent6 = extractColor(cs, "accent6").getOrElse(DefaultTheme.accentArgb(5))
