...
object DirectSaxEmitter:
  ...
  private def emitCols(writer: SaxWriter, sheet: Sheet): Unit =
    ...
    if colProps.nonEmpty then
      ...
      val sortedCols = colProps.toSeq.sortBy(_._1.index0)

      var spanStart = sortedCols.head._1
      var spanEnd = spanStart
      var spanProps = sortedCols.head._2
      ...
      sortedCols.tail.foreach { case (col, props) =>
        if col.index0 == spanEnd.index0 + 1 && props == spanProps then spanEnd = col
        else
          emitSpan()
          spanStart = col
          spanEnd = col
  ...
  private def needsXmlSpacePreserve(text: String): Boolean =
    XmlUtil.needsXmlSpacePreserve(text)
