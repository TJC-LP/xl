...
trait FunctionSpecsLogical extends FunctionSpecsBase:
   ...
   * zero/non-zero, empty FALSE, error values propagate, text refuses).
   *
    ...
    expr match
      case _: TExpr.RangeRef | _: TExpr.SheetRange =>
        evalMaybeArrayArg(ctx, expr).flatMap {
          case arr: ArrayResult =>
            rangeLogicalElements(label, arr).map(elems =>
              Option.when(elems.nonEmpty)(foldElems(elems))
            )
      ...
      case other =>
        evalMaybeArrayArg(ctx, other).flatMap {
          case arr: ArrayResult =>
            ArrayArithmetic.truthyElements(label, arr).map(v => Some(foldElems(v)))
          case scalar => scalarCondition(label, scalar).map(Some(_))
        }
  ...
  /**
   * GH-564: the logical values of a range argument, row-major. Booleans as-is, numbers
   * zero/non-zero (Excel: "if an array or reference argument contains text or empty cells, those
   * values are ignored"); an error cell propagates as its Excel error VALUE (GH-344).
   */
  private def rangeLogicalElements(
    label: String,
    arr: ArrayResult
  ): Either[EvalError, Vector[Boolean]] =
    arr.values.flatten.foldLeft[Either[EvalError, Vector[Boolean]]](Right(Vector.empty)) {
      case (Left(err), _) => Left(err)
      case (Right(acc), cv) => rangeLogical(label, cv).map(acc ++ _)
    }
  ...
  private def rangeLogical(label: String, cv: CellValue): Either[EvalError, Option[Boolean]] =
    cv match
      ...
      case _ => Right(None) // text, rich text, blank, uncached formula: ignored
  ...
  private def foldLogical(
    ...
    args
      .foldLeft[Either[EvalError, Option[Boolean]]](Right(None)) {
        case (Left(err), _) => Left(err)
        case (Right(acc), arg) =>
          conditionArg(fnName, ctx, arg)(foldElems).map {
  ...
  val and: FunctionSpec[Boolean] { type Args = BooleanList } =
    FunctionSpec.simple[Boolean, BooleanList]("AND", Arity.atLeastOne) { (args, ctx) =>
      ...
      foldLogical("AND", args, ctx, _.forall(identity), _ && _)
    }
  ...
  val or: FunctionSpec[Boolean] { type Args = BooleanList } =
    FunctionSpec.simple[Boolean, BooleanList]("OR", Arity.atLeastOne) { (args, ctx) =>
      ...
      foldLogical("OR", args, ctx, _.exists(identity), _ || _)
    }
