...
- **INDIRECT/OFFSET combined with an external ref** puts the cell in the dynamic evaluate-last bucket, whose caches are deliberately stripped for the pass — the cell then reports the external error on **every** recalculation even when Excel wrote a cache (the cache itself survives, since write-back only overlays successes).
...
- A book already poisoned by the 0.19.1 bug warm-starts **from the poison**. `IterativeCalc(maxIter, maxChange, seedFromCaches = false)` is the supported cold-start escape hatch.
