...
**Arrays in scalar function arguments (interim)**: Excel 365 applies a scalar function to each element of an array passed where it expects one value (`=ABS(C2:C4+D2:D4)` spills `{2;2;3}`, so `=SUMPRODUCT(--(B2:B4>0),ABS(C2:C4+D2:D4))` is 5). xl does not lift yet: an array-valued expression in a numeric, integer, text, boolean or date argument takes its top-left element, coerced to the argument's type (the GH-302 collapse, made type-sound — such an argument used to crash evaluation with a ClassCastException, the cause of the `view --eval` INTERNAL exit). So `=ABS(C2:C4+D2:D4)` is 2 and the SUMPRODUCT above is `#VALUE!` (its arguments differ in size) until element-wise lifting lands. The `&` operator does broadcast: `=B2:B4&"x"` is an array under `evala` and in array contexts, and its top-left element in a plain cell, like the arithmetic operators.
