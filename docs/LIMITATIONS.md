...
- ~~Sheet delete/reorder + drawing edits in one write~~ **lifted (#315)**: source mappings (drawings, comments, worksheet parts, SST accounting) are keyed by sheet NAME as read — stable under delete/reorder, re-keyed by `Workbook.rename` — so structural edits combine freely with drawing/comment edits in a single write.
