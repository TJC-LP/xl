...
**6,679 test cases** (verified via `./mill __.test`, 2026-09-08, after the #606 blind-reader fix and the #636 memory guard on the wave-28a stack): all passed; the style-performance comparison stays ignored, and four subprocess smokes (openpyxl, unwritable-directory) skip where the sandbox lacks the tool or runs as root.

| Module | Tests | Covers |
|--------|-------|--------|
| xl-evaluator | 2380 | parser, evaluator, 108-function library, dependency graph, cross-sheet formulas, recalculation, structural editing, Excel comparison total order, array CSE semantics |
| xl-core | 1533 | addressing laws, Patch/StylePatch monoids, codecs, optics, RichText, interpolation, render (HTML/SVG), styles DSL, charts, drawings, conditional formatting |
...
| xl-cli | 1285 | command parsing, batch ops, view/eval/export, streaming mode, memory guard (GH-636) |
