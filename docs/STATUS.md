...
**6,644 test cases** (verified via `./mill __.test`, 2026-09-08, on the GH-636 memory-guard branch over the wave-28a stack): all passed; the style-performance comparison stays ignored, and four subprocess smokes (openpyxl, unwritable-directory) skip where the sandbox lacks the tool or runs as root.

| Module | Tests | Covers |
|--------|-------|--------|
| xl-evaluator | 2339 | parser, evaluator, 108-function library, dependency graph, cross-sheet formulas, recalculation, structural editing, Excel comparison total order, array CSE semantics |
| xl-core | 1546 | addressing laws, Patch/StylePatch monoids, codecs, optics, RichText, interpolation, render (HTML/SVG), styles DSL, charts, drawings, conditional formatting |
| xl-ooxml | 1117 | round-trips (cells, styles, tables, comments, hyperlinks, charts, drawings, conditional formatting), compression, security (XXE, ZIP bomb), preservation |
| xl-cli | 1280 | command parsing, batch ops, view/eval/export, streaming mode, memory guard (GH-636) |
