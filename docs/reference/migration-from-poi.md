...
**A**: Mostly. Now supported, with scoped limits (see [LIMITATIONS](../LIMITATIONS.md) §12/§13): typed bar/line/pie **charts** and embedded **pictures/drawings**. Still missing: pivot tables, data-validation authoring, VBA. Formula **evaluation** is a point in XL's favor: 108 functions with whole-workbook recalculation (POI's evaluator exists but is partial and mutable). See [roadmap](../plan/roadmap.md) and [LIMITATIONS](../LIMITATIONS.md).
