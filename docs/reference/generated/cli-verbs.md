...
| verb | needs | stream | exit | batch twin | since | summary |
| --- | --- | --- | --- | --- | --- | --- |
| `rasterizers` | — | refused | 0 2 3 | — | 0.6.1 | List available SVG-to-raster backends and their status |
| `functions` | — | refused | 0 2 3 | — | 0.4.2 | List the supported Excel functions (--json: typed rows) |
...
| `sheets` | `-f` `--stream` | o1 | 0 2 3 | — | 0.1.0 | List sheets with visibility state and dimension (--stats loads the book for counts) |
...
| `names` | `-f` `--stream` | o1 | 0 2 3 | — | 0.2.0 | List defined names (named ranges) |
| `bounds` | `-f` `-s` `--stream` | o1 | 0 2 3 | — | 0.9.0 | Show the used range (instant from the dimension element; --scan for an accurate scan) |
| `view` | `-f` `-s` `--stream` | o1 | 0 1 2 3 | — | 0.1.0 | View a range as markdown, json, csv, html, svg, png, jpeg, webp or pdf (--eval, --formulas) |
| `cell` | `-f` `-s` `--stream` | o1 | 0 2 3 | — | 0.1.0 | Get one cell's value, style, comment and direct dependencies |
| `search` | `-f` `--stream` | o1 | 0 2 3 | — | 0.1.0 | Search cells by regex (all sheets unless -s; the scan stops at --limit, --total counts every match) |
| `stats` | `-f` `-s` `--stream` | o1 | 0 2 3 | — | 0.2.0 | Statistics for the numeric values in a range |
| `filter` | `-f` `-s` `--stream` | o1 | 0 2 3 | — | 0.11.3 | Filter rows of the used range with a --where predicate (read-only) |
| `describe` | `-f` `--stream` | o1 | 0 2 3 | — | 0.20.0 | Orient in a workbook: sheets, defined names, date system (--full adds per-sheet counts) |
...
| `batch` | `-f` `-s` `-o`/`-i` `--stream` | o1 | 0 1 2 3 | — | 0.1.0 | Apply multiple operations atomically from JSON (--dry-run validates; --schema prints the op schema) |
| `put` | `-f` `-s` `-o`/`-i` `--stream` | o1 | 0 1 2 3 | `put` | 0.1.0 | Write value(s) to a cell or range with smart type detection (--no-detect, --csv) |
| `putf` | `-f` `-s` `-o`/`-i` `--stream` | o1 | 0 1 2 3 | `putf` | 0.1.0 | Write formula(s) to a cell or range; one formula over a range drags with $ anchoring |
| `style` | `-f` `-s` `-o`/`-i` `--stream` | o1 | 0 2 3 | `style` | 0.2.0 | Apply formatting to cells; styles merge by default (--replace to overwrite) |
...
| `group-rows` | `-f` `-s` `-o`/`-i` | backend | 0 2 3 | `group-rows` | 0.18.0 | Group rows into a collapsible outline (--level, --collapsed) |
| `group-cols` | `-f` `-s` `-o`/`-i` | backend | 0 2 3 | `group-cols` | 0.18.0 | Group columns into a collapsible outline (--level, --collapsed) |
| `ungroup-rows` | `-f` `-s` `-o`/`-i` | backend | 0 2 3 | `ungroup-rows` | 0.18.0 | Remove outline grouping from rows |
| `ungroup-cols` | `-f` `-s` `-o`/`-i` | backend | 0 2 3 | `ungroup-cols` | 0.18.0 | Remove outline grouping from columns |
| `autofit` | `-f` `-s` `-o`/`-i` | backend | 0 2 3 | `autofit` | 0.9.6 | Auto-fit column widths from content (--columns A:F) |
| `recalc` | `-f` `-o`/`-i` | backend | 0 1 2 3 | — | 0.12.6 | Recalculate every formula and rewrite the cached values (--tables, --parallel) |
| `import` | `-f` `-s` `-o`/`-i` | backend | 0 2 3 | — | 0.7.0 | Import CSV data with type detection (--new-sheet, --delimiter, --skip-header) |
| `import-md` | `-f` `-s` `-o`/`-i` | backend | 0 2 3 | — | 0.11.3 | Import a GFM markdown table with type detection (--start, --new-sheet) |
| `add-sheet` | `-f` `-o`/`-i` | backend | 0 2 3 | `add-sheet` | 0.2.0 | Add a new empty sheet (--after, --before) |
| `remove-sheet` | `-f` `-o`/`-i` | backend | 0 2 3 | — | 0.2.0 | Remove a sheet |
| `rename-sheet` | `-f` `-o`/`-i` | backend | 0 2 3 | `rename-sheet` | 0.2.0 | Rename a sheet and rewrite every formula, defined name and chart reference to it |
| `move-sheet` | `-f` `-o`/`-i` | backend | 0 2 3 | — | 0.2.0 | Move a sheet to a new position (--to, --after, --before) |
| `copy-sheet` | `-f` `-o`/`-i` | backend | 0 2 3 | — | 0.2.0 | Copy a sheet to a new name |
| `merge` | `-f` `-s` `-o`/`-i` | backend | 0 2 3 | `merge` | 0.2.0 | Merge cells in a range |
| `unmerge` | `-f` `-s` `-o`/`-i` | backend | 0 2 3 | `unmerge` | 0.2.0 | Unmerge cells in a range |
| `comment` | `-f` `-s` `-o`/`-i` | backend | 0 2 3 | `comment` | 0.6.0 | Add a comment to a cell (--author) |
| `remove-comment` | `-f` `-s` `-o`/`-i` | backend | 0 2 3 | `remove-comment` | 0.9.6 | Remove a cell's comment |
| `clear` | `-f` `-s` `-o`/`-i` | backend | 0 2 3 | `clear` | 0.6.0 | Clear cell contents, styles or comments in a range (--all, --styles, --comments) |
| `fill` | `-f` `-s` `-o`/`-i` | backend | 0 1 2 3 | — | 0.6.0 | Fill cells with a source value or formula, Excel Ctrl+D/Ctrl+R (--right) |
| `sort` | `-f` `-s` `-o`/`-i` | backend | 0 2 3 | — | 0.6.0 | Sort rows in a range by one or more columns (--by, --then-by, --desc, --numeric, --header) |
...
| `delete-cols` | `-f` `-s` `-o`/`-i` | backend | 0 1 2 3 | — | 0.10.0 | Delete columns; shifts cells and rewrites formulas (#REF! on loss) |
| `chart add` | `-f` `-s` `-o`/`-i` | backend | 0 2 3 | `chart` | 0.12.0 | Add a typed chart built from sheet data ranges (--type, --data, --at) |
| `add-image` | `-f` `-s` `-o`/`-i` | backend | 0 2 3 | — | 0.12.0 | Embed an image (png/jpeg/gif/bmp/tiff/emf/wmf) at a cell or over a range |
| `sheet-view` | `-f` `-s` `-o`/`-i` | backend | 0 2 3 | `sheet-view` | 0.13.0 | Set sheet view options: gridlines, zoom, tab selection |
| `tab-color` | `-f` `-s` `-o`/`-i` | backend | 0 2 3 | `tab-color` | 0.13.0 | Set or clear the sheet tab color (named, #hex, rgb(), theme:accent1[:tint]) |
| `autofilter` | `-f` `-s` `-o`/`-i` | backend | 0 2 3 | `autofilter` | 0.18.0 | Set or clear the sheet-level autoFilter range |
| `page-setup` | `-f` `-s` `-o`/`-i` | backend | 0 2 3 | `page-setup` | 0.13.0 | Set print page setup: orientation, scale, fit-to-page |
| `header-footer` | `-f` `-s` `-o`/`-i` | backend | 0 2 3 | `header-footer` | 0.13.0 | Set print header/footer text with Excel codes (&L/&C/&R, &P, &N, &D, &F, &A) |
| `cf add` | `-f` `-s` `-o`/`-i` | backend | 0 2 3 | `cf` | 0.13.0 | Add a conditional-formatting rule (--range, --rule DSL, format flags) |
| `cf list` | `-f` `-s` | backend | 0 2 3 | — | 0.13.0 | List conditional-formatting rules on the sheet (read-only) |
