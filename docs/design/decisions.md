...
- **Rationale**: a cell holds one primitive, so a cell codec has nothing to derive; a row holds a product, so a row codec has everything to derive — and deriving at the row level fixes the layout too (field order = column order, field names = header row by default), which is exactly what `readRowsByHeader` and `putTable` need. A new field type costs one `given CellCodec[T]`, never a second derivation.
- **Consequence**: `Sheet.readRows` / `readRowsByHeader` / `headers` / `column` / `putRows` / `putRowsWithHeader` / `putTable` come for free from a `derives` clause; `RowCodecError.Field(row, column, field, cause)` names the cell an agent has to fix. Derivation is `inline` only for the per-field summons, the label read and the `@header` read — the codec instance is built once, non-inline — and the external-consumer probe (`xlprelude.RowCodecPreludeTest`) gates the export/inline landmine (`xl-scala-style`). The round-trip law `readRows(putRows(at, rows).dataRange) == Right(rows)` holds over generators for every codec type, required and optional (`RowCodecSpec`). The header row is `RowCodec.headers`, distinct from `fields` (GH-614): the field names by default, a field's `@header("Rev ($M)")` — a literal read off the constructor by a small `quotes.reflect` macro, refusing blanks and duplicates at compile time — or a runtime `withHeaders(Map(field -> text))` (`XLResult`, the same refusals as `InvalidArgument`) otherwise, so a tracker whose headers no identifier can spell still derives, while errors keep naming the field.
