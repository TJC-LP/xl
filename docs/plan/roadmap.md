...
| **Effect-free direct-style IO surface** (sync `Excel` without a runtime, `Iterator`-based row cursor, `boundary`-based `XLResult` block; `xl-cats-effect` kept as adapter; Ox in `xl-agent` only; ADR-017, evaluation in [design/effect-system.md](../design/effect-system.md)) | 🔵 Proposed (2026-09-07) |
