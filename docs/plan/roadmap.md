...
| iterative-recalc | [#537](https://github.com/TJC-LP/xl/issues/537), [#482](https://github.com/TJC-LP/xl/issues/482) | `jacobiFixpoint` and `dynamicCells` | Parse-once, memo on straight segments, stalled-component exit, sheets×names → names; within-SCC Gauss–Seidel by default (Excel parity), Jacobi opt-in |
...
| no-recalc-fullcalc | [#509](https://github.com/TJC-LP/xl/issues/509) | StructuralEditor cache posture | `StructuralCachePolicy.CarryForward` + `fullCalcOnLoad="1"` on `--no-recalc` structural writes |
