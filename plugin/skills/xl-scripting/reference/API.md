...
| `XLError` | Structured error enum (`SheetNotFound`, `InvalidCellRef`, `FormulaError`, ...) with `.message`; 0.22.0 adds `InvalidArgument(op, reason)` (`INVALID_ARGUMENT` — every workbook-free `Edit.validate` refusal, #617) and `NameNotFound(name, available)` (`NAME_NOT_FOUND`, with `did you mean` candidates like `SheetNotFound`, #626) |
| `RecalcResult` | `(workbook, evaluated, errors)` from `wb.recalculate()` |
| `RowCodec[A]` | one record ↔ one row of cells: `fields` (field names, column order — what errors carry), `headers` (the header row: the field names unless `@header("Rev ($M)")` on the field or `withHeaders(Map("rev" -> "Rev ($M)"))` renamed one, #614), `read(cells)`, `write(a)`; `final case class Order(...) derives RowCodec` (0.21.0). `withHeaders` is `XLResult[RowCodec[A]]` — an unknown field, a blank header, or two headers `columnOf` cannot tell apart (`RowCodec.headerKey`: case, whitespace, `_`, `-` ignored — `REV`/`rev`, `Unit Price`/`unit_price`) is `InvalidArgument`; as a given spell it `RowCodec.derived[Order].withHeaders(…)`, never `RowCodec[Order].withHeaders(…)` (summons itself) |
| `@header(name)` | annotation on a record field: its header text when the sheet's header is not an identifier (`@header("Portfolio Co.") portfolioCo: String`); a non-blank string literal, distinct per record under `RowCodec.headerKey` (the case/whitespace/`_`/`-`-insensitive key `columnOf` matches by, so `@header("Unit Price")` beside a field `unit_price` collides) — both compile errors (#614) |
...
| `sheet.readRowsByHeader[A](headerRow)` | `Either[RowCodecError, Vector[A]]` | each field's `RowCodec.headers` entry (its name, or its `@header`/`withHeaders` text) matched to header text via `columnOf` (exact, then case/space/`_`/`-`-insensitive), any column order, extra columns ignored; reads the contiguous block under the header, stops at the first blank row (0.21.0; renamed headers #614) |
...
| `sheet.columnOf(header, headerRow)` | `Option[Column]` | the column headed `header`: exact match, else case/whitespace/`_`/`-`-insensitive, leftmost wins (0.21.0); punctuation is never dropped — `Rev ($M)` is reached by `"Rev ($M)"` or `"rev ($m)"`, not by `"rev"` (hence `@header`, #614) |
...
| `sheet.toHtml(ref"A1:B10")` | `String` | inline-CSS HTML table (conditional formatting NOT painted: pass an overlay) |
| `sheet.conditionalFormatOverlay(range)` | `CfOverlay` | the sheet's conditional formatting evaluated over `range` as Excel paints it: cell-value, formula, text, top/bottom N, colour scales, 2007 data bars; precedence and Stop If True honoured; no workbook (no cross-sheet refs), system clock ([#497](https://github.com/TJC-LP/xl/issues/497)) |
| `sheet.toSvg(range, overlay)` / `sheet.toHtml(range, overlay)` | `String` | the render with the overlay painted (fills, fonts, strike, borders, number formats, data bars); the full forms take every option: `toSvg(range, includeStyles, theme, showLabels, showGridlines, overlay)`, `toHtml(range, includeStyles, includeComments, theme, applyPrintScale, showLabels, overlay)`; `CfOverlay.empty` renders byte-identically to the CF-blind call (#497) |
| `sheet.evaluateConditionalFormats(range, Some(wb), clock)` | `CfEvaluation` | `.overlay` plus `.unevaluated: Vector[CfUnevaluated]` — each rule intersecting `range` that was not painted (icon sets, above average, duplicate/unique, x14 data bars, failing formulas) or painted only where it evaluated (a formula failing for some cells: `FailedAt(cell, message, evaluatedElsewhere = true)`), with `priority`, `kind`, `ranges`, `message`. Excel's blanks/errors/dates-occurring rules paint once lifted: `sheet.copy(conditionalFormats = com.tjclp.xl.ooxml.worksheet.CfRenderLift.lift(sheet.conditionalFormats))` — render-only, never write the lifted sheet (#497) |
...
// Headers no identifier can spell (#614): @header per field, or withHeaders at runtime
final case class Deal(@header("Portfolio Co.") portfolioCo: String, @header("Rev ($M)") rev: BigDecimal, ebitda: Option[BigDecimal])
  derives RowCodec                                        // fields: portfolioCo, rev, ebitda; headers: Portfolio Co., Rev ($M), ebitda
given RowCodec[Deal] = orExit(RowCodec.derived[Deal].withHeaders(Map("ebitda" -> "EBITDA ($M)"))) // layers on @header; `derived`, not `RowCodec[Deal]`
sheet.readRowsByHeader[Deal](Row.from1(1))                  // binds Rev ($M) / EBITDA ($M) / Portfolio Co.; errors still say "rev"
```
...
| `wb("Sales")` / `wb(SheetName)` / `wb(0)` | `XLResult[Sheet]` | 0.22.0 ([#615](https://github.com/TJC-LP/xl/issues/615)): a miss is `SheetNotFound(name, available)` carrying every sheet name, so `orExit(wb("Sumary"))` prints `did you mean: Summary`; `update`/`remove`/`rename`/`setSheetState` carry the same |
...
| `Edit.validate(edit)` | `XLResult[Unit]` | the *static*, workbook-free check of one edit: value counts match the area, spans/levels/widths in range, formula syntax, `MoveSheet` names exactly one destination; 0.22.0 ([#617](https://github.com/TJC-LP/xl/issues/617)): a refusal is `XLError.InvalidArgument(op, reason)` — code `INVALID_ARGUMENT`, message `<op>: <reason>` — never `XLError.Other`, and `EditFailed` supplies `opIndex` |
...
| Style & layout | `Style(at: Area, overlay: StyleOverlay, mode: StyleMode)`, `Merge(at)`, `Unmerge(at)`, `ColWidth(sheet, cols: ColSpan, width)`, `RowHeight(sheet, rows: RowSpan, height)`, `HideCols`/`ShowCols(sheet, cols)`, `HideRows`/`ShowRows(sheet, rows)`, `AutoFit(sheet, cols: Option[ColSpan])` (0.22.0, [#613](https://github.com/TJC-LP/xl/issues/613): a column is fitted to what it DISPLAYS — a formula cell by its cached value, an uncached formula contributing nothing — so fit after `recalculate`, or let `Excel.writeChecked` supply the caches and accept the values' widths) |
...
| `wb.recalculate(IterativeCalc(maxIter, maxChange[, seedFromCaches][, scheme]))` | `RecalcResult` | 0.13.0: opt-in fixpoint for declared cycles (also `(clock, iterative)` / `(clock, rng, iterative)`); `IterativeCalc.fromCalcPr(cp)` bridges a file's `<calcPr>`. `scheme` (0.23.0): `IterationScheme.GaussSeidel` (default — Excel's sequential sweep within a cycle) or `IterationScheme.Jacobi` (previous-round reads, the pre-0.23 trajectories). `result.cycles` carries `SccReport(members, converged, rounds, maxDelta, stalled, errorValued)` |
| `result.certified` / `result.isClean` / `result.excelErrors` / `result.unconvergedVerdict` | `Boolean` / `Boolean` / `Vector[(SheetName, ARef, CellError)]` / `Option[String]` | the post-solve gate. `certified` (0.19.2) = `errors.isEmpty && converged`: a host failure (a formula that fails to parse, a missing sheet, an un-iterated cycle) or an unconverged cycle fails it. An Excel error VALUE (`#DIV/0!`, `#REF!`) is a result, not a failure (0.14.0): it sits in `excelErrors`, `certified` stays `true` — gate on `excelErrors.isEmpty` too. A cycle can converge ONTO an error value (`A1 = 1/B1`, `B1 = 1/A1`); it still certifies, and (unreleased, #678) its `SccReport.errorValued` is `true`, `result.errorValuedCycles` lists it and the summary adds `, 1 cycle settled on error values`. `SccReport.stalled` is set for a member that evaluates but fails every round (the cycle stops early, `converged = false`); a member that fails to *parse* is a host error in `errors` and never enters `cycles` |
| `wb.withCachedFormulas([clock])` | `Workbook` | = `recalculate(clock).workbook` |
| `wb.seedDataTables()` / `wb.seedDataTables(iterative)` / `wb.seedDataTables(sheet, only, clock, iterative)` | `XLResult[Workbook]` | cache every data-table interior by replaying Excel's what-if substitution (`TABLE()` is never evaluated implicitly); iterative settings derive from the book's `<calcPr>` unless given ([#453](https://github.com/TJC-LP/xl/issues/453)) |
| `wb.seedDataTablesReport()` / `wb.seedDataTablesReport(clock, iterative)` | `XLResult[DataTableSeedReport]` | 0.19.1 ([#453](https://github.com/TJC-LP/xl/issues/453)): `DataTableSeedReport(workbook, warnings: Vector[SeedTableWarning])` — a case class, not a tuple; the warnings are `CircularNotIterated`, `NotConverged`, `Skipped`, `ConeUnresolved`, `ErrorGuardFired` |
| `sheet.evaluateFormula(formula[, clock][, workbook][, currentCell])`, `sheet.evaluateFormula(formula, clock, rng[, workbook])` | `XLResult[CellValue]` | pass `Some(wb)` iff formula references other sheets; without `currentCell` it evaluates as a formula typed into a new Excel 365 cell (dynamic array, top-left value), with `Some(ref)` as the plain (legacy) formula stored at `ref` (implicit intersection) |
