...
| `result.certified` / `result.isClean` / `result.excelErrors` / `result.unconvergedVerdict` | `Boolean` / `Boolean` / `Vector[(SheetName, ARef, CellError)]` / `Option[String]` | the post-solve gate. `certified` (0.20.0) = `errors.isEmpty && converged`: a host failure (a formula that fails to parse, a missing sheet, an un-iterated cycle) or an unconverged cycle fails it. An Excel error VALUE (`#DIV/0!`, `#REF!`) is a result, not a failure (0.14.0): it sits in `excelErrors`, `certified` stays `true` — gate on `excelErrors.isEmpty` too. `SccReport.stalled` is set for a member that evaluates but fails every round (the cycle stops early, `converged = false`); a member that fails to *parse* is a host error in `errors` and never enters `cycles` |
