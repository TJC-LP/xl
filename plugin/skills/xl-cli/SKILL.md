...
| What-if without writing | `eval "=…" --with "A1=5"`, `evala "=…"` (arrays; `--at B2` anchors the displayed spill at B2) | no `-f` for constants. Both are reads: `evala --at` writes nothing, and `-o` beside it is `USAGE` (`evala is read-only and does not take -o/--output`) — `putf` writes a plain formula, which Excel evaluates with implicit intersection (see `reference/FORMULAS.md`; use SUMPRODUCT for array math in one cell) |
