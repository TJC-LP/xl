...
class BatchUpdateSpec extends FunSuite:
  ...
  test("put: single int value with auto-styles") {
  ...
  test("put: multiple values with mixed types") {
    ...
    assert(sheet.getCellStyle(ref"A1").isEmpty, "String should have no styles")
    ...
    assert(sheet.getCellStyle(ref"E1").isEmpty, "Boolean should have no styles")
  }
  ...
  test("put: unsupported types return Left (pure)") {
    ...
    result match
      ...
      case Left(other) =>
        fail(s"Expected UnsupportedType errors, got: $other")
      case Right(_) =>
  ...
  test("put: unsupported types throw with .unsafe") {
    ...
    // Should throw XLException when using .unsafe on errors result
    ...
    // Verify we can access the underlying XLError for structured errors handling
    exception.error match
      case _: XLError => // Success - errors is structured
      case other => fail(s"Expected XLError, got: $other")
  }

  test("put: writes single typed value with auto-inferred styles") {
    val sheet = Sheet("Test").getOrElse(fail("Sheet creation failed"))
      .put(ref"A1" -> BigDecimal("123.45"))
      .unsafe
  ...
  test("readTyped: returns errors for type mismatch") {
    val sheet = Sheet("Test").getOrElse(fail("Sheet creation failed"))
      .put(ref"A1", CellValue.Text("not a number"))

    val result = sheet.readTyped[Int](ref"A1")
    assert(result.isLeft, "Should return errors for type mismatch")
  }
  ...
  test("put: StyleRegistry size increases correctly") {
    val sheet = Sheet("Test").getOrElse(fail("Sheet creation failed"))
      .put(
        ref"A1" -> "No styles", // Strings don't add styles
        ref"B1" -> BigDecimal("123.45"), // Adds decimal styles
        ref"C1" -> LocalDate.of(2025, 11, 10) // Adds date styles
      )
      .unsafe
  ...
  test("put: deduplicates identical styles") {
    val sheet = Sheet("Test").getOrElse(fail("Sheet creation failed"))
      .put(
        ...
        ref"A2" -> BigDecimal("678.90") // Same type, should reuse styles
      )
      .unsafe
    ...
    assertEquals(
      ...
      "Cells with same type should have same styles"
    )
  }
