...
import com.tjclp.xl.codecs.CodecError
...
class CellCodecSpec extends ScalaCheckSuite:
  ...
  test("Double codecs: read number ref") {
    val cell = Cell(ref"A1", CellValue.Number(BigDecimal("123.45")))
    assertEquals(CellCodec[Double].read(cell), Right(Some(123.45)))
  }

  test("Double codecs: write produces number") {
  ...
  test("BigDecimal codecs: read number ref") {
  ...
  test("BigDecimal codecs: write produces number with decimal format") {
    val (value, styleOpt) = CellCodec[BigDecimal].write(BigDecimal("123.45"))
    assertEquals(value, CellValue.Number(BigDecimal("123.45")))
    assert(styleOpt.exists(_.numFmt == NumFmt.Decimal))
  }

  test("BigDecimal codecs: auto-infers decimal format") {
  ...
  test("LocalDate codecs: read datetime ref") {
  ...
  test("LocalDate codecs: read Excel serial number") {
  ...
  test("LocalDate codecs: write produces datetime with date format") {
  ...
  test("LocalDate codecs: auto-infers date format") {
  ...
  test("LocalDateTime codecs: write produces datetime with datetime format") {
    val dt = LocalDateTime.of(2025, 11, 10, 14, 30)
  ...
  test("LocalDateTime codecs: auto-infers datetime format") {
  ...
  test("Type mismatch: Int from text") {
    ...
    CellCodec[Int].read(cell) match
      ...
      case other => fail(s"Expected TypeMismatch errors, got $other")
  }

  test("Parse errors: Int from decimal") {
    val cell = Cell(ref"A1", CellValue.Number(BigDecimal("123.45")))
    CellCodec[Int].read(cell) match
