 ...
 * (`Patch.toEdits`), determinism, and all-or-nothing. Everything runs under the text-only
 * `FormulaSupport`: a law must hold whether or not the formula engine is present.
 */
class EditLawsSpec extends ScalaCheckSuite:
