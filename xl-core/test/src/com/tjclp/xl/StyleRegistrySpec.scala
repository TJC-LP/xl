...
class StyleRegistrySpec extends FunSuite:
  ...
  test("default registry contains CellStyle.default at index 0") {
    ...
    assertEquals(registry.get(0), Some(CellStyle.default))
    assertEquals(registry.indexOf(CellStyle.default), Some(0))
  }
  ...
  test("register new style appends and returns index") {
    ...
    assertEquals(idx, 1, "New style should get index 1")
    assertEquals(updated.size, 2, "Should have 2 styles now")
    assertEquals(updated.get(1), Some(boldStyle))
  }
  ...
  test("get returns None for invalid index") {
    ...
    assertEquals(registry.get(-1), None)
    assertEquals(registry.get(1), None) // Only has index 0
    assertEquals(registry.get(100), None)
  }
  ...
  test("register preserves order") {
    ...
    assertEquals(r3.get(0), Some(CellStyle.default))
    assertEquals(r3.get(1), Some(style1))
    assertEquals(r3.get(2), Some(style2))
    assertEquals(r3.get(3), Some(style3))
  }
  ...
  test("register complex style with all components") {
    ...
    assertEquals(idx, 1)
    assertEquals(registry.get(1), Some(complexStyle))
    assertEquals(registry.indexOf(complexStyle), Some(1))
  }
  ...
  test("multiple registrations of same style return same index") {
    ...
    assert(indices.forall(_ == 1), "All registrations should return index 1")
