...
class WorkbookModificationSpec extends FunSuite:
  ...
  private val path = Files.createTempFile("workbooks-mod", ".xlsx")

  override def afterAll(): Unit =
    Files.deleteIfExists(path)

  private val ctx = SourceContext.fromFile(path, PartManifest.empty, SourceFingerprint.fromPath(path))
  private val baseSheet = Sheet("Sheet1").fold(err => fail(s"Failed to create sheets: $err"), identity)
  ...
  test("delete tracks deletions") {
    val sheet2 = Sheet("Sheet2").fold(err => fail(s"Failed to create sheets: $err"), identity)
  ...
  test("put marks sheets as modified when replacing existing sheets") {
    val modifiedSheet = baseSheet.put(ref"A1" -> "New Value").fold(err => fail(s"Failed to modify sheets: $err"), identity)
    val updated = workbook.put(modifiedSheet).fold(err => fail(s"Put failed: $err"), identity)
    val tracker = updated.sourceContext.fold(fail("Missing source context"))(identity).modificationTracker
    assertEquals(tracker.modifiedSheets, Set(0))
  }

  test("put does not mark as modified when adding new sheets") {
    val newSheet = Sheet("Sheet2").fold(err => fail(s"Failed to create sheets: $err"), identity)
