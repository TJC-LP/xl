...
/** Bidirectional codecs for cell values */
...
object CellCodec:
  ...
  /** String codecs - reads text and number cells as strings */
  ...
  /** Int codecs - reads numeric cells as integers */
  ...
  /** Long codecs - reads numeric cells as longs */
  ...
  /** Double codecs - reads numeric cells as doubles */
  ...
  /** BigDecimal codecs - reads numeric cells with auto-inferred decimal format */
  ...
  /** Boolean codecs - reads boolean cells */
  ...
  /**
   * LocalDate codecs - reads date/datetime cells and Excel serial numbers with auto-inferred date
   * format
   */
  ...
  /**
   * LocalDateTime codecs - reads date/datetime cells and Excel serial numbers with auto-inferred
   * datetime format
   */
  ...
  /**
   * RichText codecs - reads rich text cells or converts plain text to RichText.
   *
   * Note: Rich text has formatting within the text runs (intra-cell formatting), not at the cell
   * level. Therefore, this codecs does not return a CellStyle hint (formatting is in the TextRun
   * font properties).
   */
  inline given CellCodec[RichText] = codec(
    ...
    rt => (CellValue.RichText(rt), None) // No cell-level styles, formatting is in runs
  )
