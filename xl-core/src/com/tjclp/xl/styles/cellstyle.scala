...
/**
 * Complete cell styles combining all formatting aspects.
 */
...
case class CellStyle(
  ...
  /**
   * Memoized canonical key for styles deduplication.
   *
   * Two styles with the same key are structurally equivalent and should map to the same styles
   * index in styles.xml. Computed once and cached for performance.
   */
...
object CellStyle:
  ...
  /**
   * Generate canonical key for styles deduplication (backward compatibility).
   *
