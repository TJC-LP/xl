...
object HtmlRenderer:
   ...
   *
   * Generates a `<table>` element with cells converted to `<td>` elements. If includeStyles is true, cell styles are
   * converted to inline CSS. Rich text cells are rendered with HTML formatting tags (<b>, <i>, <u>, <span>).
   *
   * @param sheet
   *   The sheet to export
   * @param range
  ...
  def toHtml(
    ...
    val tableRows = cellsByRow.map { (rowNum, rowCells) =>
...
$tableRows
   ...
   *
   * Generates CSS properties for font, fill, alignment, etc. Returns empty string if cell has no style.
   */
  private def cellStyleToInlineCss(cell: Cell, sheet: Sheet): String =
    cell.styleId.flatMap(sheet.styleRegistry.get).map { style =>
      ...
      if style.font.sizePt != Font.default.sizePt then
        css += s"font-size: ${style.font.sizePt}pt"
      if style.font.name != Font.default.name then
        css += s"font-family: '${style.font.name}'"
