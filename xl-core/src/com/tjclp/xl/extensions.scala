 ...
 *
 * Extensions return [[XLResult]] for chainable errors handling. Use `.unsafe` once at the end of
 * chains to unwrap.
 *
 ...
 *   .styles("A1:B1", boldStyle)
 *   .unsafe  // Single boundary point
 * }}}
 *
 * '''Styling approaches:'''
 *   - `.styles(range, styles)` - Template pattern (format before data)
 *   - `.put(ref, value, styles)` - Inline styling
...
object extensions:
  ...
  extension (sheet: Sheet)
    /**
     * Apply styles to cell or range (template pattern).
     *
     * ✅ Restored clean name (was .applyStyle)
     *
     * @param ref
  ...
  extension (sheet: Sheet)
     ...
     *
     * ✅ Restored clean name (was .getCell)
     *
     * @param cellRef
     ...
     *
     * ✅ Restored clean name (was .getCells)
     *
     * @param rangeRef
     *   Range like "A1:B10"
     * @return
     ...
     */
    def range(rangeRef: String): Option[List[Cell]] =
      CellRange
        .parse(rangeRef)
        .toOption
  ...
  extension (result: XLResult[Workbook])
    ...
    /** Remove sheets by name (chainable). */
  ...
  extension (workbook: Workbook)
    /** Get sheets by name (safe). */
    def sheet(name: String): Option[Sheet] =
