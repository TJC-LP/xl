...
case class Sheet(
   ...
   *
   * Sheet-qualified refs (Sales!A1) ignore the sheets name and use only the cell/range part.
   *
  ...
  /**
   * Batch put with mixed value types and automatic styles inference.
   *
   * Accepts (ARef, Any) pairs and uses runtime pattern matching to resolve the appropriate codecs
   ...
   * RichText, Formatted. Unsupported types return Left with errors.
   *
   * Example:
   * {{{
   * sheets.put(
   ...
   * sheets.put(ref"A1" -> "Hello").unsafe
   * }}}
   *
   * @param updates
   *   Varargs of (ARef, Any) pairs
   * @return
   *   Either errors (if unsupported type) or updated sheets
   */
  @SuppressWarnings(Array("org.wartremover.warts.Var"))
  def put(updates: (ARef, Any)*): XLResult[Sheet] =
    import com.tjclp.xl.codecs.{CellCodec, given}
   ...
   *
   * This avoids runtime type inspection and lets the compiler inline the codecs implementation,
   ...
   *   The patch to apply
   * @return
   *   Either an updated sheets or an errors
   */
