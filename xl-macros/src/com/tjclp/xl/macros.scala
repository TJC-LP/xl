...
object macros:
  ...
  private def cellImpl0(sc: Expr[StringContext])(using Quotes): Expr[ARef] =
    ...
    try {
      ...
      case e: IllegalArgumentException => report.errorAndAbort(s"Invalid cell literal '$s': ${e.getMessage}")
    }
  ...
  private def rangeImpl0(sc: Expr[StringContext])(using Quotes): Expr[CellRange] =
    ...
    try {
      ...
      case e: IllegalArgumentException => report.errorAndAbort(s"Invalid range literal '$s': ${e.getMessage}")
    }

  private def errorNoInterpolation(sc: Expr[StringContext], args: Expr[Seq[Any]], kind: String)(using Quotes): Expr[Nothing] =
  ...
  private def parseCellLit(s: String): (Int, Int) = {
    ...
    while (i < n && s.charAt(i).isDigit) { row = row * 10 + (s.charAt(i) - '0'); i += 1; sawDigit = true }
